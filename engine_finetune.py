...
def train_ft(model, optimizer, device, train_loader, vali_loader, test_loader, path_ft, args):
    ...
    for epoch in range(args.n_epochs_finetune):
        ...
        if args.n_epochs_finetune > 1:
            vali_loss_epoch = vali_ft(model, vali_loader, criterion, device, args)
        else:
            vali_loss_epoch = 0.0
        test_loss_epoch = 0.0
        ...
        loss_dict = {'train_loss': train_loss_epoch, 'valid_loss': vali_loss_epoch}
...
def vali_ft(model, vali_loader, criterion, device, args):
    model.eval()
    vali_loss = []
    with torch.no_grad():
