...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.mask_token_type = args.mask_token_type
    ...
    def random_mask(x, mask_ratio):
        ...
        index_keep = index_shuffle[:, :len_keep] 
    ...
    def forward_pt(self, x):
        ...
        x_patch_vis = torch.gather(x_patch, dim=1, index=index_keep.unsqueeze(-1).repeat(1, 1, self.patch_len))  # bs * n_vis * p, shuffle

        mask2, index_shuffle_back2, index_keep2, index_remove2 = self.random_mask(x_patch_vis, self.mask_ratio)
        x_patch_vis_vis = torch.gather(x_patch_vis, dim=1, index=index_keep2.unsqueeze(-1).repeat(1, 1, self.patch_len))  # bs * n_vis_vis * p, shuffle shuffle
        ...
        x_patch_in = torch.gather(x_patch_in, dim=1,
                                  index=index_shuffle_back2.unsqueeze(-1).repeat(1, 1, self.patch_len))  # shuffle, unshuffle

        x_patch_embd = self.patch_embedding(x_patch_in)

        x_patch_pe = self.position_embedding(x_patch)
        x_patch_pe_vis = torch.gather(x_patch_pe, dim=1, index=index_keep.unsqueeze(-1).repeat(1, 1, self.d_model))  # shuffle, unshuffle
        ...
        # return loss & acc
        return loss, torch.zeros(1, device=loss.device), 1., model_visual_outs
    ...
    def forward_ft(self, x):
        ...
        # enc_in = x_patch + x_patch_pe
        enc_in = self.dropout(x_patch + x_patch_pe)

        enc_out, attn = self.enc(enc_in)  # bs * n * d_model == (bs*c) * n * d_model
    ...
    def forward_ck(self, x):
        ...
        x_patch, c_in = self.do_patch(x)
        x_patch = self.patch_embedding(x_patch)
        x_patch_pe = self.position_embedding(x_patch)
    ...
    def forward_eval(self, x):
        ...
        x_patch_vis = torch.gather(x_patch, dim=1, index=index_keep.unsqueeze(-1).repeat(1, 1, self.patch_len))

        mask_token = torch.zeros((x_patch_vis.shape[0], index_remove.shape[1], self.patch_len), requires_grad=True,
                                 device=x_patch_vis.device)

        x_patch_in = torch.cat([x_patch_vis, mask_token], dim=1)
        x_patch_in = torch.gather(x_patch_in, dim=1,
                                  index=index_shuffle_back.unsqueeze(-1).repeat(1, 1, self.patch_len))
