...
from engine_pretrain import train_pt, test_pt
from engine_finetune import train_ft, test_ft, test_zeroshot
...
parser.add_argument('--pos_mask_ratio', type=float, default=0.4, help='PE masking ratio for the unmaksed patch')
parser.add_argument('--mask_token_type', type=str, default='zeros', help='option: [zeros, params]')
...
if args.mode == 'pt':
    ...
    path = os.path.join('./checkpoints', settings_pt)
    ...
    path_ft = os.path.join('./checkpoints', settings_ft)
    if not os.path.exists(path_ft):
        os.mkdir(path_ft)
